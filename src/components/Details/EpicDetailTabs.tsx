...
import { Epic, ProjectDocument, Story, ReviewRequest, ReviewComment } from '@/lib/types';
...
type TabType = 'detail' | 'documents' | 'stories' | 'review-requests' | 'review-comments';

export default function EpicDetailTabs({ epic, projectId, onUpdate }: EpicDetailTabsProps) {
  const { showConfirm, handleStoryClick } = useProject();
  ...
  // Detail Form State
  const [editEpic, setEditEpic] = useState<Epic>(epic);
  ...
  const fetchDocuments = async () => {
    setIsLoading(true);
    try {
      const data = await documentService.getDocuments('EPIC', epic.id);
      setDocuments(data || []);
  ...
  const fetchStories = async () => {
    setIsLoading(true);
    try {
      const data = await storyService.getStoriesByEpic(epic.id);
      setStories(data || []);
    } catch (error) {
      toast.error('Không thể tải danh sách Story');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchReviewRequests = async () => {
    setIsLoading(true);
    try {
      const data = await reviewRequestService.getReviewRequestsByEpic(epic.key);
      const sortedData = (data || []).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      setReviewRequests(sortedData);
    } catch (error) {
      toast.error('Không thể tải danh sách Review Request');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const fetchReviewComments = async () => {
    setIsLoading(true);
    try {
      const data = await reviewCommentService.getReviewCommentsByEpic(epic.key);
      const sortedData = (data || []).sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
  ...
  const handleUpdateEpic = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const updated = await epicService.updateEpic(epic.id, editEpic);
  ...
  return (
    <div className={styles.taskDetailTabs}>
      <div className={styles.tabsHeader}>
        ...
        <button 
          className={`${styles.tabItem} ${activeTab === 'stories' ? styles.tabItemActive : ''}`}
        ...
        <button 
          className={`${styles.tabItem} ${activeTab === 'review-requests' ? styles.tabItemActive : ''}`}
      ...
      <div className={styles.tabsContent}>
        {activeTab === 'detail' && (
          <form onSubmit={handleUpdateEpic} className={styles.detailForm}>
            <div className={styles.formGroup}>
              <label>Tiêu đề Epic</label>
              <input 
                type="text" 
                value={editEpic.title} 
                onChange={e => setEditEpic({...editEpic, title: e.target.value})}
                required
              />
            </div>
            <div className={styles.formGrid}>
              <div className={styles.formGroup}>
                ...
                <select 
                  ...
                  onChange={e => setEditEpic({...editEpic, status: e.target.value as any})}
                >
              ...
              <div className={styles.formGroup}>
                ...
                <select 
                  ...
                  onChange={e => setEditEpic({...editEpic, priority: e.target.value as any})}
                >
            ...
            <div className={styles.formGroup}>
              ...
              <input 
                type="date" 
                value={editEpic.deadline ? new Date(editEpic.deadline).toISOString().split('T')[0] : ''} 
                onChange={e => setEditEpic({...editEpic, deadline: e.target.value})}
              />
            </div>
            <div className={styles.formGroup}>
              <label>Mô tả</label>
        ...
        {activeTab === 'stories' && (
          <div className={styles.taskList} style={{ padding: '0 16px 16px' }}>
            {isLoading ? (
              <div className={styles.loadingState}><Loader2 size={24} className={styles.spin} /></div>
            ) : stories.length === 0 ? (
              <div className={styles.emptyState}>Chưa có Story nào trong Epic này</div>
            ) : (
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)', backgroundColor: 'var(--background)' }}>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Key</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Tiêu đề</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Trạng thái</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Độ ưu tiên</th>
                    </tr>
                  </thead>
                  <tbody>
                    {stories.map(s => (
                      ...
                      >
                        <td style={{ padding: '12px 16px', fontWeight: '500' }}>{s.key}</td>
                        <td style={{ padding: '12px 16px' }}>{s.title}</td>
                        <td style={{ padding: '12px 16px' }}>
                          <span className={`${styles.statusBadge} ${styles[s.status?.toLowerCase()] || ''}`}>{s.status}</span>
                        </td>
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ 
                            fontSize: '11px', 
                            ...
                            padding: '4px 8px', 
                            borderRadius: '4px',
                            backgroundColor: s.priority === 'HIGH' ? 'rgba(239, 68, 68, 0.1)' : s.priority === 'MEDIUM' ? 'rgba(245, 158, 11, 0.1)' : 'rgba(16, 185, 129, 0.1)',
          ...
          <div className={styles.docContainer} style={{ marginTop: 0, height: '80vh', minHeight: 'auto' }}>
            <div className={styles.docSidebar} style={{ width: '35%' }}>
              <div className={styles.tabActions} style={{ marginBottom: '12px' }}>
                <button className={styles.uploadBtnMini} onClick={() => document.getElementById('epic-file-upload')?.click()}>
                ...
                <input 
                  ...
                  onChange={async (e) => {
                    if (e.target.files) {
                      try {
                        setIsLoading(true);
                        const newDocs = await documentService.uploadDocuments('EPIC', epic.id, Array.from(e.target.files));
        ...
        {activeTab === 'review-requests' && (
          <div className={styles.taskList} style={{ padding: '0 16px 16px' }}>
            ...
            {isLoading ? (
              ...
              <div className={styles.emptyState}>Chưa có Review Request nào</div>
            ) : (
              <div style={{ overflowX: 'auto', backgroundColor: 'var(--surface)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)', backgroundColor: 'var(--background)' }}>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Key</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Trạng thái</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Người yêu cầu</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Reviewer</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Ngày tạo</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)', textAlign: 'right' }}>Thao tác</th>
                    </tr>
                  </thead>
                  <tbody>
                      ...
                      <tr key={rr.id} style={{ borderBottom: '1px solid var(--border)' }}>
                        ...
                        <td style={{ padding: '12px 16px' }}>
                          <span style={{ 
                            ...
                            fontSize: '11px',
                            fontWeight: '600',
                            backgroundColor: rr.status === 'APPROVED' ? 'rgba(16, 185, 129, 0.1)' : rr.status === 'REJECTED' ? 'rgba(239, 68, 68, 0.1)' : 'rgba(107, 114, 128, 0.1)',
                            color: rr.status === 'APPROVED' ? '#10b981' : rr.status === 'REJECTED' ? '#ef4444' : '#6b7280',
                            border: `1px solid ${rr.status === 'APPROVED' ? 'rgba(16, 185, 129, 0.2)' : rr.status === 'REJECTED' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(107, 114, 128, 0.2)'}`
                          }}>{rr.status}</span>
                        </td>
                        <td style={{ padding: '12px 16px' }}>{rr.requester}</td>
                        <td style={{ padding: '12px 16px' }}>{rr.reviewer}</td>
                        <td style={{ padding: '12px 16px', color: 'var(--text-secondary)' }}>
                          {new Date(rr.createdAt).toLocaleDateString()} {new Date(rr.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                        </td>
                        <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                          <div style={{ display: 'flex', gap: '8px', justifyContent: 'flex-end', alignItems: 'center' }}>
                            ...
                            {rr.comment && (
                              <button 
                                className={styles.submitBtn} 
                                style={{ padding: '4px 12px', fontSize: '12px', minWidth: 'auto', width: 'auto', display: 'inline-block' }}
        ...
        {activeTab === 'review-comments' && (
          <div className={styles.taskList} style={{ padding: '0 16px 16px' }}>
            ...
            {isLoading ? (
              ...
              <div className={styles.emptyState}>Chưa có Review Comment nào</div>
            ) : (
              <div style={{ overflowX: 'auto', backgroundColor: 'var(--surface)', borderRadius: '8px', border: '1px solid var(--border)' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px', textAlign: 'left' }}>
                  <thead>
                    <tr style={{ borderBottom: '1px solid var(--border)', backgroundColor: 'var(--background)' }}>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Mã</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Tiêu đề</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Trạng thái</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Reviewer</th>
                      <th style={{ padding: '12px 16px', fontWeight: '500', color: 'var(--text-secondary)' }}>Ngày tạo</th>
                        ...
                        <tr key={rc.id} style={{ borderBottom: '1px solid var(--border)' }}>
                          ...
                          <td style={{ padding: '12px 16px' }}>
                            <span style={{ 
                              ...
                              color: statusColor,
                              border: `1px solid ${rc.status === 'APPROVED' ? 'rgba(16, 185, 129, 0.2)' : rc.status === 'REJECTED' ? 'rgba(239, 68, 68, 0.2)' : 'rgba(156, 163, 175, 0.2)'}`
                            }}>{rc.status}</span>
                          </td>
                          <td style={{ padding: '12px 16px' }}>{rc.reviewer}</td>
                          <td style={{ padding: '12px 16px', color: 'var(--text-secondary)' }}>
                            {new Date(rc.createdAt).toLocaleDateString()} {new Date(rc.createdAt).toLocaleTimeString([], {hour: '2-digit', minute:'2-digit'})}
                          </td>
                          <td style={{ padding: '12px 16px', textAlign: 'right' }}>
                            <button 
                              className={styles.submitBtn} 
      ...
      {selectedStory && (
        <div className={`${styles.slideOverOverlay} ${styles.open}`} style={{ zIndex: 1000 }} onClick={() => setSelectedStory(null)}>
          <div className={styles.slideOverPanel} onClick={(e) => e.stopPropagation()}>
            <div className={styles.slideOverHeader}>
              <div className={styles.slideOverHeaderLeft}>
                <Book size={16} />
                <span>{selectedStory.key}: {selectedStory.title}</span>
              </div>
